...
public class HbaseTest {
    ...
    public void createTable() throws Exception{
        ...
        //描述表
        //设置表名
        TableName tableName = TableName.valueOf("student_new_3");
        ...
        admin.createTable(tableDescriptor,"1".getBytes(),"8".getBytes(),4);
    }
    ...
    /**
     * 过滤器:
     *      select * from student where name='zhangsan-1'
     * @throws Exception
     */
    @Test
    public void filterEquals() throws Exception{
        //获取table对象
        Table table = connection.getTable(TableName.valueOf("student"));
        //封装数据
        Scan scan = new Scan();
    ...
    /**
     *  select * from student where name like 'zhansan%'
     */
    @Test
    public void filterLike() throws Exception{
        Table table = connection.getTable(TableName.valueOf("student"));

        Scan scan = new Scan();
        SingleColumnValueFilter filter = new SingleColumnValueFilter("base_info".getBytes(), "name".getBytes(), CompareOperator.EQUAL, new SubstringComparator("zhangsan"));
        //如果数据没有指定要查询的列,则忽略不要
     ...
     */
    @Test
    public void filterByRowkey() throws Exception {

        Table table = connection.getTable(TableName.valueOf("student"));

        Scan scan = new Scan();
        //根据rowkey过滤数据
        RowFilter filter = new RowFilter(CompareOperator.EQUAL, new BinaryPrefixComparator("1001".getBytes()));
        scan.setFilter(filter);
    ...
    //组合条件查询
    // select * from student where address=shenzhen or ( name='zhansan-5' and age=25)
    @Test
    public void filterList() throws Exception{
        ...
        SingleColumnValueFilter nameFilter = new SingleColumnValueFilter("base_info".getBytes(), "name".getBytes(), CompareOperator.EQUAL, "zhangsan-5".getBytes());
        SingleColumnValueFilter ageFilter = new SingleColumnValueFilter("base_info".getBytes(), "age".getBytes(), CompareOperator.EQUAL, Bytes.toBytes(25));


        //address=shenzhen
        SingleColumnValueFilter addressFilter = new SingleColumnValueFilter("base_info".getBytes(), "address".getBytes(), CompareOperator.EQUAL, "shenzhen".getBytes());
        addressFilter.setFilterIfMissing(true);
        //name='zhansan-5' and age=25
        FilterList nameAndAgeFilter = new FilterList(FilterList.Operator.MUST_PASS_ALL);
        //name='zhansan-5'
        nameAndAgeFilter.addFilter(nameFilter);
        //age=25
        nameAndAgeFilter.addFilter(ageFilter);
        //address=shenzhen or ( name='zhansan-5' and age=25)
        FilterList filterList = new FilterList(FilterList.Operator.MUST_PASS_ONE);
        //
        filterList.addFilter(addressFilter);
        filterList.addFilter(nameAndAgeFilter);
