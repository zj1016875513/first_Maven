...
//        String[] strArray = list.toArray(new String[list.size()]);  //list带类型转数组
// 创建一个顺序流
...
//    3、使用Stream的静态方法：of()、iterate()、generate()
    ...
    public void flatmap(){
        ...
        Stream<String> flatmapStream = list.stream().flatMap(x -> {
            String[] s = x.split(" ");
            return Arrays.stream(s); //拍平了再返回一个流
        });
    ...
    public void filter(){
        ...
        System.out.println("---------------");
        personList.stream().sorted((a,b)->(a.getSalary()-b.getSalary())).forEach(System.out::println);

        System.out.println("---------------");
        //可以从结果中看出，因为多线程的原因，forEach这种方式排序就不准确了 ，需要forEachOrdered
        personList.parallelStream().sorted((a,b)->(a.getSalary()-b.getSalary())).forEachOrdered(System.out::println);
    ...
    public void map_flatmap(){
        ...
        Map<String, Integer> mapcollect = Arrays.stream(strArr).collect(Collectors.toMap(x -> x, x -> 1));
        //不收集就只能是流
...
//        collect主要依赖java.util.stream.Collectors类内置的静态方法。
        ...
        System.out.println("tomap"+mapcollect);
    ...
    public void reduce(){
        ...
        // 求和方式2
        Optional<Integer> sum2 = list.stream().reduce((a, b) -> Integer.sum(a, b));
        // 求和方式3
    ...
    public void salary(){
        ...
        // 求工资之和方式3：
        Integer sumSalary3 = personList.stream().reduce(0, (sum, p) -> sum += p.getSalary(), (a, b) -> Integer.sum(a, b));
    ...
    @Test
    public void partition_grouping(){
        List<Person> personList = new ArrayList<Person>();
        personList.add(new Person("Tom", 8900, 23, "male", "New York"));
        personList.add(new Person("Jack", 7000, 25, "male", "Washington"));
        personList.add(new Person("Lily", 7800, 21, "female", "Washington"));
    ...
    @Test
    public void distinct_limit_skip(){
        String[] arr1 = { "a", "b", "c", "d" };
        String[] arr2 = { "d", "e", "f", "g" };
